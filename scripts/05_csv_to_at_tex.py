...
def parse_ref(ref: str) -> tuple[int, int]:
    ...
    return ch_s, int(v_digits) if v_digits else 0
...
def main():
    for job, output in jobs:
        ...
        with outfile(output).open("w", encoding="utf-8") as out:
            current_ch = None
            current_at_ch = None
            versecount = 0
            for r in rows:
                txt=r["at_text"]
                if not txt.strip(): # ignore empty text
                    continue
                at_ch = r["at_ch"]
                v = r["v"]
                ch = r["ch"]
                ...
                cont = r["cont"]
                txt=r["at_text"]
                if at_ch != current_at_ch:
                    if current_at_ch is not None:
                        out.write("\\par\n")
                    out.write(f"\\ChapterHeading{{{at_ch}}}\n")
                    current_at_ch = at_ch
                    current_ch = at_ch
                elif ch != current_ch:
                    if current_ch is not None:
                        out.write("\\par\n")
                    out.write(f"\\ChapterParenHeading{{{ch}}}\n")
                    current_ch = ch

                txt = render_structured_to_latex(inject_latex_footnotes(render_markers(wrap_hebrew(texify_double_quotes(esc(txt))))))
                out.write(f"\\Verse{{{ref}}}{{{v}}}{{{cont}}}{{{par}}}{{{txt}}}{{{mt_ref}}}\n")
